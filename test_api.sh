...
echo -e "${YELLOW}Test 13: Try to add invalid course (invalid status)${NC}"
response=$(curl -s -X POST $BASE_URL/api/courses \
    -H "Content-Type: application/json" \
    -d '{
...
echo "Response: $response"
if [[ $response == *"success\":false"* ]] && [[ $response == *"Status must be one of"* ]]; then
        print_test 0 "Invalid status error handling works correctly"
else
        print_test 1 "Should reject invalid status value"
fi
echo ""

echo -e "${YELLOW}Test 14: Try to add invalid course (invalid date format)${NC}"
response=$(curl -s -X POST $BASE_URL/api/courses \
    -H "Content-Type: application/json" \
    -d '{
        "name": "Invalid Date Course",
        "description": "This should fail",
        "target_date": "08-01-2026",
        "status": "Not Started"
    }')
echo "Response: $response"
if [[ $response == *"success\":false"* ]] && [[ $response == *"target_date must be in YYYY-MM-DD format"* ]]; then
        print_test 0 "Invalid date format error handling works correctly"
else
        print_test 1 "Should reject invalid date format"
fi
echo ""

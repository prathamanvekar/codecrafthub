...
def load_courses():
    ...
    if not os.path.exists(DATA_FILE):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to create data file: {e}") from e
        return []
    ...
    try:
        ...
        raise RuntimeError('Data file is not valid JSON')
    except OSError as e:
...
def is_valid_date(date_value):
    """
    Validate date string format as YYYY-MM-DD.
    """
    try:
        parsed_date = datetime.strptime(date_value, '%Y-%m-%d')
        # Ensure strict format validation (no partial or alternate formats)
        return parsed_date.strftime('%Y-%m-%d') == date_value
...
def validate_course_data(data, required_fields):
    ...
    # Validate target completion date format if provided
    if 'target_date' in data and not is_valid_date(data['target_date']):

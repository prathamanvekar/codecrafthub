...
curl -X POST "$BASE_URL/api/courses" \
  ...
  -d '{
    ...
    "description": "Build beginner APIs with Flask",
    "target_date": "2026-12-15",
    "status": "Not Started"
  }'
```

Expected: `201 Created` with `"success": true` and an auto-generated `id` and `created_at`.
...
Expected: `200 OK` with `count` and `courses` array.
...
Expected: `200 OK` with `"course"` object when ID exists.
...
Expected: `200 OK` with `"success": true` and updated `course` object.
...
Expected: `200 OK` with `"success": true` and `deleted_course` object.

6. Get stats (GET)

```bash
curl "$BASE_URL/api/courses/stats"
```

Expected: `200 OK` with `total_courses` and `by_status` counts.
...
Expected: `400 Bad Request` with error like `Missing required fields`.
...
Expected: `400 Bad Request` with error `target_date must be in YYYY-MM-DD format`.

4. Course not found

```bash
curl "$BASE_URL/api/courses/9999"
```

Expected: `404 Not Found` with error indicating the course ID was not found.
